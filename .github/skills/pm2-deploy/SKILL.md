...
**On Windows**, `pm2 deploy` requires `sh` on the PATH. Prepend Git Bash's bin directory before running:

```powershell
$env:PATH = "C:\Program Files\Git\bin;$env:PATH"; pm2 deploy ecosystem.config.cjs production
```
